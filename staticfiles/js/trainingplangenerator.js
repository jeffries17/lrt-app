      ...
      taperWeeks.forEach((week, index) => {
        ...
        taperHtml += `
          <tr>
            ...
            <td>${(parseFloat(week.easyRunDistance) + parseFloat(week.mediumRunDistance) + parseFloat(week.speedRunDistance) + parseFloat(week.longRunDistance)).toFixed(2)}${unitValue}</td>
          </tr>
        `;
      });
      return taperHtml;
...
function extractPlanData() {
  ...
  trainingPlanOutput.querySelectorAll('.week').forEach((weekElement, index) => {
      const weekNumber = index + 1;
      const days = [];

      weekElement.querySelectorAll('.day').forEach(dayElement => {
          const dayNameElement = dayElement.querySelector('.day-name');
          const activityElement = dayElement.querySelector('.activity');

          if (dayNameElement && activityElement) {
              const day = dayNameElement.innerText;
